...
from metrics import compute_isi_score, compare_templates, make_interp_temp
from merges import ID_merge_groups, make_merges, compute_overall_score
from id_noise_templates import id_noise_templates, find_depth

def postprocessing(folder):

    merge_threshold = 2.0
    distance_to_compare = 6

    output_directory = os.path.join(folder, 'automerge')
    ...
    np.save(os.path.join(output_directory,'spike_clusters.npy'), clusters)
    np.save(os.path.join(output_directory,'comparison_matrix.npy'), comparison_matrix)