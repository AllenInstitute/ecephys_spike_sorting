...
def calculate_waveform(exptpath, nBoots =100):
    ...
    rawDataFile = glob.glob(exptpath+'/experiment'+'*'+'-0_0.dat')[0]
    clustersFile = os.path.join(exptpath,'spike_clusters.npy')
    spikeTimesFile = os.path.join(exptpath,'spike_times.npy')
    clusterGroupsFile =     os.path.join(exptpath,'cluster_group.tsv')
    
    waveformsFile = os.path.join(exptpath, 'mean_waveforms.npy')
    SNRFile = os.path.join(exptpath, 'SNR.npy')
    templatesFile = os.path.join(exptpath, 'templates.npy')
    ...
    #                SNR_boots[i,:,:]=signaltonoise(waveforms, axis=2)
    #                waveform_boots[i,:,:]=np.nanmean(waveforms,2)   
    #            mean_waveforms[cluster_num, :, :] = np.squeeze(np.mean(waveform_boots,0))  
    #            SNR=np.squeeze(np.mean(SNR_boots,0))    
    #        else:
    #            waveforms = np.zeros((samplesPerSpike, numChannels, TW))
    #            for wv_idx in range(0, TW):
    #                peak_time = times_for_cluster[wv_idx][0]
    ...
    #                except ValueError:
    #                    waveforms[:, :, wv_idx] = np.nan      
    #            mean_waveforms[cluster_num, :, :] = np.nanmean(waveforms,2)
    #            SNR=signaltonoise(waveforms, axis=2)
